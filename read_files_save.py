...
def write_feedback(file,feedback,f,filename,grader,email):
    ...
    for cell in q13m:
        write_feedback_line('Missing term from explanation {} -{:.2f}'.format(''.join(q13m[cell]),sum_deduction_lists([q14m],md14)),feedbackfile)
    ...
    out36_2 = {'3.3.6':['.003','0.002']}
    q36o_2 = check_in_code_output(get_sub_dict('3.3.5',markdown),file,solution,out35)
    if len(q36o_2) == 1:
        q36o_2 = {}
    deduct36 = sum_deduction_lists([q36o],out36) + sum_deduction_lists([q36o_2],out36_2)
    
           
    q33 = [key for key in points.keys() if '3.3.' in key]
    total = sum([points[q] for q in q33])
    grade -= (deduct31+deduct32+deduct33+deduct34+deduct35+deduct36)
...
#        files = os.listdir(dirc+'/'+grader+'/'+f)
        files = os.listdir(dirc+'/'+f)
        files = get_notebooks(files)
        files = check_notebook(dirc,f,files)
        
        
        file, filename=open_notebook(dirc,f,files)
